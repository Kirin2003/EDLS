...
public class CIP_Main {
    public static void main(String[] args) {
        ...
        for (int r = 0; r < instanceNumber; r++){
            ...
            environment.createType2(4000, 1600, 1, 1);
            ...
            IdentifyTool cls = new ECLS(logger,recorder,environment);
            cls.execute();
            environment.reset();
            for(Reader_M reader_m : environment.getReaderList()) {
                reader_m.recorder = new Recorder();
            ...
            IdentifyTool sfmti = new SFMTI(logger,recorder,environment);
            sfmti.execute();
            environment.reset();
            for(Reader_M reader_m : environment.getReaderList()) {
                reader_m.recorder = new Recorder();
