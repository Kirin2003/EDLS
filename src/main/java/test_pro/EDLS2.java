 ...
 */
public class EDLS2 extends IdentifyTool {
    /**
     * 哈希函数的个数, 用于意外标签去除阶段
     */
     ...
     */
    public EDLS2(Logger logger, Recorder recorder, Environment environment) {
...
//        }
            //TODO 测试用!
        Reader_M reader = environment.getReaderList().get(0);
            logger.error("<<<<<<<<<<<<<<<<<<<< 阅读器: " + reader.getID() + " >>>>>>>>>>>>>>>>>>>");
            // 每一次需要reset(),将环境中所有预期标签设为活跃, 因为有些标签在其他阅读器中识别为缺失, 但在某个阅读器中识别为存在, 这个标签是存在的
            environment.reset();
            identify( reader);
