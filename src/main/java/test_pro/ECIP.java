...
public class ECIP extends IdentifyTool {
    ...
    public void execute() {
        ...
        int missNum = expectedCidNum - recorder.actualCids.size();
        recorder.correctRate = 1-(expectedCidNum-expectedActualCidNum-missNum)*1.0/expectedCidNum;
        ...
        System.out.println("时隙:"+recorder1.roundSlotCountList);
        ...
        System.out.println("expectedCidNum expectedActualCidNum miss_recognize"+expectedCidNum+" "+expectedActualCidNum+" "+missNum);
        System.out.println("准确率:"+recorder.correctRate);
    }

    public void identify() {
        for (Reader_M reader : environment.getReaderList()) {
            logger.error("<<<<<<<<<<<<<<<<<<<< Reader: " + reader.getID() + " >>>>>>>>>>>>>>>>>>>");
            // 将环境中所有预期标签设为活跃, 因为有些标签在其他阅读器中识别为缺失, 但在某个阅读器中识别为存在, 这个标签是存在的
            environment.reset();
            identify( reader);
        }
    }
    ...
    public void identify(Reader_M reader_m) {
        ...
        List<Tag> responseTagList = reader_m.getCoverActualTagList(); // 真实标签+没有去除的意外标签
        boolean flag = false; // 是否循环

        Set<String> expectedCidSet = new HashSet<>();
        Set<String> actualCidSet = new HashSet<>();
        environment.getExpectedTagList().forEach(tag -> expectedCidSet.add(tag.categoryID));
        int unReadCidNum = expectedCidSet.size();
        int expectedCidNum = unReadCidNum;
        reader_m.getCoverActualTagList().forEach(tag->actualCidSet.add(tag.getCategoryID()));
        int actualCidNum = actualCidSet.size();
        int coveredActualCidNum = actualCidNum; // expectedCidNum和actualCidNum用于计算准确率
        ...
        responseTagList.addAll(environment.getAllTagList());
        // 测试用
        List<Tag> unexpect = new ArrayList<>();
        unexpect.addAll(environment.getAllTagList());
        ...
        System.out.println("###");
        unexpect.forEach(tag -> System.out.println(tag.getSlotSelected()));
...
//        slots.sort((integer, t1) -> integer > t1 ? 1 : -1);
        ...
        for (Integer slotId : CidMap.keySet()) {
            ...
            if (l == 1) {
                slotToTagList.get(slotId).forEach(tag -> tag.setActive(false));
        ...
        while(flag) {
            ...
            System.out.println("###"+unexpect.size());
            unexpect.forEach(tag -> System.out.println(tag+" "+tag.getSlotSelected()));
            // 5 标签选择时隙(重排)
            ...
            for(Tag tag : responseTagList) {
                if (tag.isActive()) {
                    ...
                    System.out.println(tag+" "+oldslot);
                    System.out.println(unexpect.contains(tag));
                    // 加一条假设!TODO
//                    if(!indicator.containsKey(oldslot))continue;
...
//            slots2.sort((integer, t1) -> integer > t1 ? 1 : -1);
            ...
            recorder1.recognizedActualCidNum += readCidNumInOneRound;
            recorder1.recognizedCidNum += readCidNumInOneRound;
            logger.info("第 ["+(recorder1.roundCount+1)+"] 轮,识别到的存在的类别数: ["+readCidNumInOneRound + " ],缺失的类别数: [0], 花费的时间:["+executionTimeOneRound2+"] ms");
            ...
            int missingnum = expectedCidSet.size()-recorder1.actualCids.size();
            recorder1.recognizedMissingCidNum += missingnum;

            recorder1.recognizedMissingCidNumList.set(recorder1.recognizedMissingCidNumList.size()-1,missingnum);
            recorder1.recognizedCidNum += recorder1.missingCids.size();
            int recognizedCidNumLastRound = recorder1.recognizedCidNumList.get(recorder1.recognizedCidNumList.size()-1)+missingnum;
            ...
            // 计算准确率
            recorder1.correctRate = 1-(expectedCidNum-coveredActualCidNum-missingnum)*1.0/expectedCidNum;
        System.out.println("unReadCidNum actualCidNum missingnum"+unReadCidNum+" "+actualCidNum+" "+missingnum);
        System.out.println("识别错误的类别数"+(unReadCidNum-actualCidNum-missingnum)+"阅读器准确率:"+recorder1.correctRate);
    }
